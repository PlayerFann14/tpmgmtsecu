...
- 10 risques identifiés · validation humaine obligatoire · 52 tests
...
- Puis phase **validation humaine** : 10 décisions a/d/c → `valide_par` signé
- 🎤 Notes « Lancement réel. Le simulateur reproduit notre référence : on peut donc vérifier la cohérence sans clé API. » *(4 min)*
...
```
[détectée] ignore_instructions   extrait='tes instructions'
[détectée] override_system       extrait='Tu es maintenant'
... 4 motifs détectés, neutralisés en [[DONNEE-NON-EXECUTABLE]]
```
- ▲ Journalisé (`injection_detectee`) **avant** le 1er appel d'agent
...
- Totaux R-01→R-10 : **trouvés 10/10 · inventés 0 · écarts de niveau 0**
- 🎤 Notes « La matrice déterministe et le grounding RAG expliquent cet accord : le niveau n'est jamais laissé à l'IA. » *(3 min)*
...
- **10/10 risques retrouvés** vs la référence manuelle · **0 fabrication** · injection **résistée**
