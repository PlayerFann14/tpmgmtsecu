...
- [ ] Cas d'étude : A, B ou C ? (B → LINDDUN devient pertinent en plus de STRIDE)
- [ ] Modèle de menaces de base : STRIDE par défaut ? (LINDDUN en complément si B)
- [ ] Notation : matrice P×I, ou DREAD, ou CVSS ? Justifier.
- [ ] Qualitatif seul ou hybride (quelques ALE pour les actifs critiques) ?
- [ ] Forme du prototype : Python + API LLM / framework d'agents / no-code ?
- [ ] Modèle(s) LLM utilisés — à **citer** dans le dossier.
- [ ] Répartition des 4 membres sur les 5 jalons.
