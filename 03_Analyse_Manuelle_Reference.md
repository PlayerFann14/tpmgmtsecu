...
**Résiduel** : aucun risque résiduel n'est « faible » après traitement — il est
**toujours non nul** et devra être **accepté explicitement par la direction**
(cours p. 54 : *« la direction doit l'accepter de façon explicite et formelle »*).
...
### R-04 — Exfiltration / perte des dossiers (A-01)
```
AV  = 300 000 €    reconstruction + sanction + contentieux (part estimée)
EF  = 60 %         perte de 60 % de la valeur : données irrécupérables / fuite
SLE = AV × EF = 180 000 €
ARO = 0,5          un évènement tous les 2 ans
ALE = SLE × ARO = 90 000 € / an      ← ce que le risque coûte en moyenne par an

Mesure proposée : sauvegardes immuables hors ligne + test mensuel
ALE(after) = 15 000 €      ACS = 12 000 €
Valeur = 90 000 − 15 000 − 12 000 = 63 000 € > 0  →  MESURE RENTABLE ✅
```
